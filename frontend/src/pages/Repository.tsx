...
const Repository: React.FC = () => {
    ...
    const { data: datasetPage, isLoading, error } = useQuery({
        ...
        queryFn: () => datasetApi.getAllDatasets(0, 20, category || undefined)
    });
            ...
            <Grid container spacing={4}>
                ...
                <Grid item xs={12} md={3}>
                    <Card sx={{ p: 2, borderRadius: 3, boxShadow: '0 4px 6px rgba(0,0,0,0.02)' }}>
                        ...
                        {category ? (
                            <Box>
                                <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>Active Category:</Typography>
                                <Chip 
                                    label={category} 
                                    onDelete={() => navigate('/datasets')} 
                                    color="secondary" 
                                    sx={{ fontWeight: 600 }}
                                />
                            </Box>
